...
A `when` container is the bounded canonical-state visibility primitive. The original truthy form `{ kind: "when", binding: "active", children: [...] }` compiles to MorphTile's native `{ group: [...], when: ["var", "active"] }`. A common numeric-threshold form can additionally author `comparison` plus finite numeric `threshold`, for example `{ kind: "when", binding: "energy", comparison: "at_least", threshold: 10, children: [...] }`. The only threshold comparisons are `above`, `at_least`, `below`, and `at_most`, compiled directly to MorphTile's native `>`, `>=`, `<`, and `<=` expressions. The binding still uses `intent.bindings.readouts` / `readout_logic_vars`; the threshold is authored configuration and the live value remains canonical target state. Half-specified threshold contracts, unsupported comparisons, non-numeric thresholds and snapshot-shaped extras HOLD. The machine does not expose arbitrary expression authoring.
...
- threshold `when` authoring may carry only one of four fixed comparisons plus one finite numeric threshold; arbitrary expression trees remain outside the contract;
...
10. **When placement cannot be satisfied:** invalid, unknown, or mode-inert presentation descriptors return `HOLD_INVALID_PRESENTATION_PLACEMENT` rather than being silently retained or discarded.
