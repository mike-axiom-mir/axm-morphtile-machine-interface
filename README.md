...
Supported element kinds are deliberately bounded to MorphTile-native `text`, `repeat_text`, `readout`, `meter`, `control`, `action`, `tile`, `row`, `group`, `when`, `repeat`, and `repeat_when` nodes. `row`, `group`, `when`, `repeat`, and `repeat_when` recursively contain `children`, giving creation-time relative/conditional/repeated layout without inventing a private pixel/layout or general expression authority. Authored order is preserved at every level.
...
Inside a `repeat`, `repeat_when` is the bounded nearest-repeat lexical selector. The equality shorthand is `{ kind: "repeat_when", source: "index"|"count", equals: N, children: [...] }`. The relational form uses `comparison` plus integer `value`, with only `above`, `at_least`, `below`, `at_most`, `equals`, and `not_equals`. These compile directly to MorphTile's native scoped comparisons over lexical repeat locals `i` / `i_of`. `index` values are bounded to `0..nearest-repeat max-1`; `count` values are bounded to `1..nearest-repeat max`. Nested selectors bind to the nearest repeat. These lexical locals are runtime scope, not canonical state, so they create no readout proof obligation. Half-specified/mixed forms, fractional or out-of-range operands, unsupported operators, and authority-shaped extras HOLD.

A view title may also read one canonical target variable through `intent.title_binding`. That field is mutually exclusive with static `intent.title`, must be declared in `intent.bindings.readouts`, and compiles only to MorphTile's native `view.title: ["var", name]` expression. The live title is resolved by MorphTile at render time, so Interface carries the symbolic name and target-proof obligation but never snapshots the value or exposes arbitrary title expressions.

MorphTile core now also evaluates expression-backed text plus readout/meter/button/control labels in the nearest repeat scope. Interface consumes that public primitive through a closed presentation vocabulary rather than exposing arbitrary expressions. `{ kind: "repeat_text", source: "index"|"count", prefix?, suffix?, strong? }` compiles only to a native text expression over `i` / `i_of`. A `readout`, `meter`, `action`, or `control` inside a repeat may use `repeat_label: { source: "index"|"count", prefix?, suffix? }` instead of a static `label`. Repeat-local text/labels outside a lexical repeat fail closed with `HOLD_INTERFACE_REPEAT_SCOPE`; `label` and `repeat_label` are mutually exclusive. The lexical value decorates presentation only: it does not add a canonical-state proof dependency, does not copy a repeat local into matter, and does not widen the signal or parameter authority of the action/control it labels. For `readout`, the lexical descriptor decorates only the label; its `binding` remains the same canonical target-owned readout variable.
...
- `title_binding` plus readout/meter/when/repeat/action/control names must be explicitly declared symbolic bindings, including when nested inside row/group/when/repeat layout;
...
1. **What it does:** Builds candidate ordered/nested relative, bounded canonical-state conditional/repeated, bounded repeat-local selective/presentational, bounded tile-composed, and bounded statically styled views plus canonical presentation descriptors for an existing tile.
