...
- ASSEMBLY RECEIVER TARGET: exact integrated Assembly `9fe8b53daf3e572c596c142859cca2199a976553`; compatibility is claimed only for exact-head CI that checks out this receiver identity. The receiver proof includes the producer-generated v0.5 presentation positive-control specimen that distinguishes omission, authored `false`, and numeric zero, and the Assembly input set includes a valid status-less plain-map candidate wrapper so the current wrapper-shape contract is exercised rather than inferred.
