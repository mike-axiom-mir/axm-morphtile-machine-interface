...
## v0.5 bounded relative, conditional and repeated layout
...
Supported element kinds are deliberately bounded to MorphTile-native `text`, `readout`, `meter`, `control`, `action`, `row`, `group`, `when`, and `repeat` nodes. `row`, `group`, `when`, and `repeat` recursively contain `children`, giving creation-time relative/conditional/repeated layout without inventing a private pixel/layout or expression authority. Authored order is preserved at every level.
...
1. **What it does:** Builds candidate ordered/nested relative, bounded conditional and bounded repeated views plus canonical presentation descriptors for an existing tile.
...
8. **Layout boundary:** row/group/when/repeat are structural presentation containers only. They do not create state, permissions, host windows, absolute pixels, or action authority.
...
- PINNED INTEGRATION HARNESS: executes ordered/nested view, meter, conditional visibility, bounded repeat and placement behavior through real MorphTile clone → plan → commit → receipt → rollback, proves native row/group materialization through `compilePanel`, proves generated meters/conditionals/repeats follow canonical variables at render time, and proves canonical parameter values remain unchanged by interface generation.
