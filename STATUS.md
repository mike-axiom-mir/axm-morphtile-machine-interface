...
The claim registry currently covers action input authority, Assembly dependency folding and import-plan coverage, disjoint binding namespaces, bounded canonical truthy/threshold/equality conditions, meters/readouts, basic commit/rollback, nested target paths, presentation HOLDs and host modes, repeat-local selectors, canonical repeat generation, shared canonical controls, local/same-root tile composition and native view styling. Claim identity records proof meaning only; it does not make a test CANON or widen runtime authority.
